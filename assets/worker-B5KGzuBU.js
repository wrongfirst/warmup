(function(){var e=``+new URL(`yaegi-gBnBKIsY.wasm`,self.location.href).href;function t(e){Promise.resolve().then(()=>e.init?.()).then(()=>{self.postMessage({type:`READY`})}).catch(e=>{console.error(`[Worker Init Error]:`,e),self.postMessage({type:`INIT_ERROR`,error:e?.message||String(e)})}),self.onmessage=async t=>{let n=t.data;if(!n||n.type!==`RUN`)return;let{id:r,userCode:i,testCode:a=``}=n;try{let t=await e.execute(i,a);self.postMessage({type:`RESULT`,id:r,success:t.success,output:t.output,error:t.error})}catch(e){self.postMessage({type:`RESULT`,id:r,success:!1,output:``,error:e?.message||String(e)})}}}function n(e){let t=new Set;if(!e||!e.trim())return{imports:t,body:``};let n=e.split(`
...
`).trim()}}let r=null;function i(){return r||=n(`package main
...
`)}`}async function o(e){if(typeof self.yaegiEval==`function`){let t=self.yaegiEval(e);return{success:!!t?.success,output:t?.output||``,error:t?.error||void 0}}throw Error(`WASM interpreter binary (yaegi.wasm) is not loaded.`)}t({async init(){if(i(),(0,eval)(`// Copyright 2018 The Go Authors. All rights reserved.
