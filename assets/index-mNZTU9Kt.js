...
ALL TESTS PASSED!`;let n=t.includes(e);X.getState().markComplete(e),n?ET(`Passed!`):gD()}setRunningState(e){this.isRunning=e,$.runBtn.disabled=e||!this.isReady,e?($.runBtn.classList.add(`run-btn-fill`),$.runBtn.innerHTML=`<span>${bD.STOP}</span><span>Run</span>`):($.runBtn.classList.remove(`run-btn-fill`),$.runBtn.innerHTML=`<span>${bD.PLAY}</span><span>Run</span>`)}waitForCompiler(){setInterval(async()=>{let e=yD.getInitError?.();if(e){this.isReady=!1,fD.setError(),this.setRunningState(this.isRunning),(!$.console.textContent||$.console.textContent===`// Ready...`)&&($.console.textContent=`${yD.name.toUpperCase()} runtime initialization failed:\n${e}`);return}let t=await yD.isReady();t!==this.isReady&&(this.isReady=t,t&&fD.setReady(),this.setRunningState(this.isRunning))},500)}};function SD(){let{title:e,subtitle:t,headline:n,logo_image:r,logo_emoji:i}=my;document.title=n||(t?`${e} | ${t}`:e),$.branding.logo&&(r?$.branding.logo.innerHTML=`<img src="${r}" alt="Logo" class="h-8 w-auto" />`:$.branding.logo.textContent=i),$.branding.title&&($.branding.title.textContent=e),$.branding.subtitle&&($.branding.subtitle.textContent=t)}function CD(e,t,n,r){e&&(e.innerHTML=t.map(e=>`<div class="px-8 py-1 pb-0 text-[10px] font-bold text-fg-muted uppercase">${e.title}</div>`+e.exercises.map(e=>{let t=r.includes(e.id);return`<div class="nav-item cursor-pointer py-2 pl-8 pr-4 text-sm flex justify-between items-center transition-colors ${e.id===n?`bg-bg-surface text-fg-primary border-l-2 border-brand`:`text-fg-muted hover:text-fg-primary`} ${t?`opacity-40`:``}"
